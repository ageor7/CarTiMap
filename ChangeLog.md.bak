...
# CHANGELOG: [v8.13.48b] — 2026-09-08 — MapViewer v6.4.36, ASTCompiler v1.2.16, AppOrchestrator v3.7.42 [PUBLISHED]
- **ES6 Destructuring Standardization [REF: COD-02]:** Standardized array destructuring across all coordinate regex matches and date splitters to prevent intermediate platform parsers from stripping active numerical bracket values.
- **Base-Template Syntax Termination [REF: CRASH-10]:** Resolved the boot-blocking unterminated string literal crash near line 1574 by surgically closing the single-quote sentinel on the AppOrchestrator version tracking parameter, unblocking the browser's compiler.
...
- **CMS Transport Ingestion Guard [REF: CRASH-02]:** Restored primary data ingestion (`fetchUrlPrimary`) and secondary basemaps config (`fetchUrlBasemaps`) transport channels to the CORS-relaxed Google Visualization API queries (`/gviz/tq?tqx=out:csv`). This bypasses direct un-CORSed `/export` downloads to secure remote origins.
- **Flex Sticky Modal Geometry [REF: UI-176b]:** Restructured the Search and About modals using flexbox constraints. Locked close buttons (`[X]`) in a stationary, top-right anchor while delegating scrollability strictly to nested body viewports (`overflow-y: auto`).
...
- **L2228 Syntax Error Cleared:** Purged a stray escaped newline sequence (`\n`) from the `showButtonText` state hook initialization closure.
...
- **Micro-Typography Telemetry:** Redesigned the scale metrics with condensed label spacing, preventing text truncation in narrow or portrait device viewports.

### Fixed
- **Athens Sewer WKT Re-rendering:** Resolved a major mapping bug where multi-line polygon coordinates in location rows (specifically representing subterranean aqueduct and conduit networks) were fragmented, failing to display on map launch.
- **Greek Dialect Search Compatibility:** Standardised coordinate string parsing to be case-insensitive, ensuring that mixed-case entries natively render as geographical overlays.
- **Double Taskbar Border Clashing:** Cleared the duplicate borders that cluttered the bottom status cockpit. Integrated the horizontal resizer directly into the top edge of the navigation panel, producing a clean, single-pixel interactive divider.
- **Current Slide Numeric Steppers:** Suppressed browser-native numeric steppers/spinner arrows within the slide select box, securing a stable vertical height for the status bar.
...
### Changed
- **One-Line Navigation Cockpit:** Collapsed the status/toolbar to a single-line height of 38px to maximize spatial-temporal canvas viewports. Enlarged interactive buttons and text inputs by one click to protect accessibility on tablet-class screens. Shifted dynamic timeline scale metrics directly to the immediate right of the Next control button [REF: UI-164c].
- **Multi-Platform Geometry Splitting:** Re-engineered the MapViewer coordinate ingestion engine to recursively split multi-line cells using both HTML breaks and platform-native newline configurations (`\r\n`, `\n`, `\r`) [REF: MAP-01f].

### Restored
- **High-Fidelity Branding Suite:** Re-integrated the original master vector logo (the bracket base, sapphire primary needle, active-green secondary needle, and sub-pixel glow filter) inside the `<CarTiMapperLogo>` component [REF: UI-236b].

### Removed
- **Unsanitized WKT Fallbacks:** Purged fragile regex fallbacks for malformed coordinates, enforcing strict WKT compliance with real-time logging feedback.

## [v8.13.16] by Gemini Pro
- Removed the // comments from inside <${TimelineScrubber}

## [v8.13.15-telemetry] — 2026-08-30 - AppOrchestrator v3.7.13, MapViewer v6.4.22, TimelineScrubber v26.11.8 [CONFORMED]
### Fixed
- **Status Bar Telemetry Disconnect:** Integrated the missing `.semantic-time-span-left` HTML rendering block inside the left status container (`status-left`) of the parent layout. This ensures the parent component physically projects the scrubber's dynamically calculated timeline zoom-span telemetry (`visibleTimeSpan`), correcting a silent state-hoisting dead end without requiring modification of existing functional React hooks or prop-routing bridges [REF: UI-164b].
...
- **Unified Version Promotion:** Aligned and promoted the master baseline suite to version v8.13.6, incrementing AppOrchestrator to v3.7.6 and MapViewer to v6.4.22 to guarantee absolute synchronization with live GitHub.
- **MediaViewer Synchronization:** Aligned the main AppLayout block with the newly conformed `MediaViewer` block on GitHub, purging the legacy `MediaScroller` layout references.

## [v8.13.5] — 2026-08-29 - AppOrchestrator v3.7.5 [REGRESSED]
### Broken
- **CORS Ingestion Exception:** An uncoordinated codebase update migrated the primary Google Sheets fetch URL to `/export?format=csv`. Because Google's raw export endpoint does not set the required Access-Control-Allow-Origin headers, this change completely broke anonymous client-side fetches across differing web origins, triggering catastrophic browser CORS violations [REF: CRASH-06].
...
**Refinements & Specifications:**
   ...
   ✓ **[ResizeObserver Integration]** Linked container width measurements directly to `ResizeObserver` listener to guarantee highly responsive axis tick calculations on container size changes.
